...
void AKCGameMode::RestoreSlotDataForController(AController* Controller)
{
	...
	if (!KCPS)
	{
		UE_LOG(LogKCLobby, Warning, TEXT("[KC_TRACE][Spawn] RestoreSlotDataForController 중단 - KCPS가 null. Controller=%s"),
	...
	if (!SessionSub)
	{
		UE_LOG(LogKCLobby, Warning, TEXT("[KC_TRACE][Spawn] RestoreSlotDataForController 중단 - SessionSub가 null."));
		return;
	}
	...
	UE_LOG(LogKCLobby, Warning, TEXT("[KC_TRACE][Spawn] RestoreSlotDataForController 진행 중 - NetIdStr='%s', PlayerName='%s', QueryKey='%s'"),
		*NetIdStr, *KCPS->GetPlayerName(), *QueryKey);

	if (QueryKey.IsEmpty())
	{
		UE_LOG(LogKCLobby, Warning, TEXT("[KC_TRACE][Spawn] RestoreSlotDataForController 중단 - QueryKey가 비어있음."));
		return;
	}
...
{
	UE_LOG(LogKCLobby, Warning, TEXT("[KC_TRACE][Spawn] UpdatePlayerStartSpot 진입, Player=%s"), Player ? *Player->GetName() : TEXT("null"));

	RestoreSlotDataForController(Player);

	UE_LOG(LogKCLobby, Warning, TEXT("[KC_TRACE][Spawn] UpdatePlayerStartSpot 완료, Player=%s"), Player ? *Player->GetName() : TEXT("null"));
